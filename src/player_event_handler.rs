...
use librespot::{
    ...
    playback::player::{PlayerEvent, SinkStatus},
};
use librespot_playback::player::PlayerEventListener;

pub struct EventHandler {
    onevent: String,
}

impl PlayerEventListener for EventHandler {
    fn is_valid(&self) -> bool {
        true // we could invalid it when onevent can't be called
    }
    ...
    fn on_event(&mut self, event: &PlayerEvent) {
        ...
        match event {
            ...
            PlayerEvent::TrackChanged { audio_item } => match audio_item.track_id.to_base62() {
                ...
                Ok(id) => {
                    ...
                    env_vars.insert("URI", audio_item.uri.clone());
                    env_vars.insert("NAME", audio_item.name.clone());
                    env_vars.insert(
                        "COVERS",
                        audio_item
                            .covers
                    ...
                    match audio_item.unique_fields {
                        UniqueFields::Track {
                            ...
                            env_vars.insert("ALBUM", album.clone());
                        ...
                        UniqueFields::Episode {
                            ...
                            env_vars.insert("DESCRIPTION", description.clone());
                            env_vars
                                .insert("PUBLISH_TIME", publish_time.unix_timestamp().to_string());
                            env_vars.insert("SHOW_NAME", show_name.clone());
                        }
                    }
                }
            },
            ...
            PlayerEvent::TimeToPreloadNextTrack { track_id, .. } => match track_id.to_base62() {
                Err(e) => warn!("PlayerEvent::TimeToPreloadNextTrack: Invalid track id: {e}"),
                Ok(id) => {
            ...
            PlayerEvent::SessionConnected {
                ...
                env_vars.insert("CONNECTION_ID", connection_id.clone());
                env_vars.insert("USER_NAME", user_name.clone());
            }
            ...
            PlayerEvent::SessionClientChanged {
                ...
                env_vars.insert("CLIENT_ID", client_id.clone());
                env_vars.insert("CLIENT_NAME", client_name.clone());
                env_vars.insert("CLIENT_BRAND_NAME", client_brand_name.clone());
                env_vars.insert("CLIENT_MODEL_NAME", client_model_name.clone());
            }
        ...
        if !env_vars.is_empty() {
            run_program(env_vars, &self.onevent);
        }
    }
}

impl EventHandler {
    pub fn new(onevent: &str) -> Self {
        Self {
            onevent: onevent.to_string(),
        }
    }
}
